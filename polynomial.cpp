...
class poly
{
	...
	public:
		...
		void insert(int c,int p)
		{
			...
			if(head==NULL)											//for adding a single element
			{
				head=a;
			}
			else if(a->pow>head->pow && head->next==NULL)			//for adding a node after head
				{
					head->next=a;
					a->next=NULL;
				}
		    else if (a->pow<head->pow && head->next==NULL)			//for adding a node before head
				{
					  a->next=head;
					  head->next==NULL;
					  head=a;
